<template>
  ...
  <table v-if="searchedUser==null" class="w-full">
    <tr class="text-4xl" v-for="user in users" :key="user" :class="{ focused: user === searchedUser }">
      <td class="py-2">{{ user }}</td>
      <td class="py-2">
        <button @click="deleteUser(user)"
                class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-3 rounded-full cursor-pointer">
          Borrar usuario
        </button>
      </td>
    </tr>
  ...
  <teleport v-if="deletedUser!==null" to="body">
    <confirm-message :message="'Estás seguro de que quieres borrar la cuenta ' +deletedUser"
      ...
      <button @click="deleteUserCommit"
              class=" text-center text-4xl bg-red-500 text-white font-bold py-2 px-3 rounded hover:bg-red-600 transition-colors duration-300">SI
      </button>
      <button
              @click="this.deletedUser=null"
              class=" bg-gray-300 text-gray-700 font-bold text-4xl text-center py-2 px-3 ml-4 rounded hover:bg-gray-400 transition-colors duration-300">NO
      </button>
    </confirm-message>
  </teleport>
  </div>
